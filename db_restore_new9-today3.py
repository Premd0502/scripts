...
for project in projects:
    ...
    if restore_status == "success":
        try:
            ...
            # --- Drop database after health check ---
        try:
            subprocess.check_call(
                f"mysql -u{mysql_root_user} -p'{mysql_root_pass}' -e 'DROP DATABASE IF EXISTS `{db_name}`'", shell=True
            )
            log(f"Dropped database: {db_name}")
        except Exception as e:
            log(f"Failed to drop database {db_name}: {e}")
